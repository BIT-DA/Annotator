        ...
        for cur_it in range(total_it_each_epoch):
            ...
            with amp.autocast(enabled=self.if_amp):
                ...
                if cfgs.MODEL.NAME in ['MinkNet', 'SPVCNN']:
                    ...
                    voxel_labels = batch['targets'].F.long().cuda(non_blocking=True)
                    network_loss = ret_dict['network_loss']
                    loss = network_loss(voxel_predict_logits, voxel_labels)
                elif cfgs.MODEL.NAME in ['Polarnet']:
                    predict_logits = ret_dict['predict_logits']
                    label = batch['processed_label'].long()
                    network_loss = ret_dict['network_loss']
                    loss = network_loss(predict_logits, label)
    ...
    def train(self):
        ...
        with tqdm.trange(
            ...
            for cur_epoch in tbar:
                ...
                if (trained_epoch % self.ckp_save_interval == 0 or trained_epoch == self.total_epoch) and self.rank == 0:
