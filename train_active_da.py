...
class Trainer:
    ...
    def __init__(self, args, cfgs):
        ...
        souce_dataset, source_loader, source_sampler = build_dataloader(
            data_cfgs=cfgs.SOURCE_DATA,
            modality=cfgs.MODALITY,
            batch_size=cfgs.OPTIM.BATCH_SIZE_PER_GPU,
            dist=self.if_dist_train,
            workers=args.workers,
        ...
        self.source_train_set = souce_dataset
        ...
        self.source_epoch = cfgs.OPTIM.SOURCE_EPOCH
        self.model = model
    ...
    def train_one_epoch(self, tbar, data_cfg):
        ...
        source_dataloader_iter = iter(self.source_loader)

        if self.sampler is not None:
            self.sampler.set_epoch(self.cur_epoch)
            self.source_sampler.set_epoch(self.cur_epoch)
        ...
        for cur_it in range(total_it_each_epoch):
            ...
            source_batch = next(source_dataloader_iter, None)
            ...
            with amp.autocast(enabled=self.if_amp):
                ...
                source_ret_dict = self.model(source_batch)
                if cfgs.MODEL.NAME in ['MinkNet', 'SPVCNN']:
                    ...
                    if self.cur_epoch < self.source_epoch:
                        source_voxel_predict_logits = source_ret_dict['voxel_predict_logits']
                        source_voxel_label = source_batch['targets'].F.long().cuda(non_blocking=True)
                        source_network_loss = source_ret_dict['network_loss']
                        source_loss = source_network_loss(source_voxel_predict_logits, source_voxel_label)
                        loss = network_loss(voxel_predict_logits, voxel_label) + source_loss
                    else:
    ...
    def evaluate(self, dataloader, prefix):
        ...
        for k, batch_dict in enumerate(dataloader):
...
def main():
    ...
    if cfgs.WANDB.USE_WANDB:
        wandb.init(
            ...
            name=f'{proctitle}_{args.extra_tag}',
