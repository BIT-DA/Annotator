...
class SempossVoxelDataset(data.Dataset):
    def __init__(
            self,
            data_cfgs=None,
            training=True,
        ...
        self.point_cloud_dataset = SemanticpossDataset(
            data_cfgs=data_cfgs,
            training=training,
    ...
    def get_single_sample(self, index, voting_idx=0):
        ...
        lidar_mapped = SparseTensor(feat_, pc_)
        labels = SparseTensor(labels, pc)
        labels_ = SparseTensor(point_label, pc_)
        ret = {
            'name': pc_data['path'],
            'lidar': lidar,
            'lidar_mapped': lidar_mapped,
            'targets': labels,
    ...
    def collate_batch(inputs):
        ...
        for sample in inputs:
            collate_list.append({'name': sample['name'],
                                 'lidar': sample['lidar'],
                                 'lidar_mapped': sample['lidar_mapped'],
        ...
        ret['raw_coord'] = [sample['raw_coord'] for sample in inputs]
        return ret
